...
def test_run_exploit_forwards_nonce_only_on_attack(monkeypatch):
    """_run_exploit splices the verification nonce into the sandbox CLI only on the
    live 'attack' phase, so run_sandbox injects HALO_NONCE for the output gate; the
    network-less 'test' phase must never carry --nonce. This is the one splice hop
    otherwise covered only by inspection."""
    captured = {}
    ...
    ex._run_exploit({"code": "print(1)", "phase": "attack",
                     "target": "10.0.0.1", "nonce": "cafe01"})
    assert "--nonce cafe01" in captured["command"]
    assert "--target 10.0.0.1" in captured["command"]

    ex._run_exploit({"code": "print(1)", "phase": "test", "nonce": "cafe01"})
    assert "--nonce" not in captured["command"]


def test_run_exploit_schema_declares_nonce():
    """The run_exploit inputSchema must declare `nonce` (optional) — _run_exploit
    reads p.get('nonce') and the whole breach-verification gate depends on it being
    forwarded. Insurance against a future additionalProperties:false."""
    schema = next(t["inputSchema"] for t in halo_tools.TOOLS
                  if t["name"] == "run_exploit")
    assert "nonce" in schema["properties"]
    assert "nonce" not in schema.get("required", [])
