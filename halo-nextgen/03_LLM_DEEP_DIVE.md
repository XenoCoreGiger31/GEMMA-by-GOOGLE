# 03 — Frontier Models of Code & Hacking: Model Intelligence for HALO

This is **not** a survey of "every LLM ever."
...
- **Lens A — Build HALO's brain:** which frontier models to cast as HALO's *actor*,
...
> Gemma; the operator also runs two **Qwen** phone quants. So "frontier-tier" here
...
## Part A — The frontier tier (the only models in scope)
...
| Model family | Why it's frontier | Open weights / local? | Best HALO role |
|---|---|---|---|
...
| **DeepSeek V3 / R1** | Open **reasoning** frontier model. R1's chain-of-thought rivals closed frontier on math/code; V3 is a strong MoE coder. Distills run locally. | **Yes** — full + R1-distill (Qwen/Llama) variants | **Local decision-brain** (R1-distill) |
...
**Explicitly excluded:** 1–3B toy chat models,
...
| Role | Job | Frontier pick | Local pick (runnable) |
|---|---|---|---|
...
- The **judge** is a *different* model with intact safety training and no tool
  ...
  judge (`02` §C5).
- The **decider** is the reasoning model that turns "we ran tools" into "here's
  what's exploitable, proven" (`01`, `05`). This is the upgrade that moves HALO
  from a tool-runner to a decision engine.
...
beat a frontier adversary, not a script kiddie:
...
- **Autonomous exploit reasoning:** frontier + R1-class models decompose CVEs,
  ...
  model IQ.
- **Code understanding at scale:** frontier coders read a repo and find the bug
...
the same frontier models. HALO's moat is the **harness**: validated exploitability,
tiered memory, tunable autonomy, chain of custody, injection defense, and debug
soundness. That is the whole thesis of this package.

---

## Part D — Harness lessons from the frontier models (esp. Claude Code)

These models' *harnesses* are as important as their weights. What HALO borrows:
...
- **Decider:** reasoning model (DeepSeek-R1 local; Claude if policy allows) —
  turns tool output into proven exploitability.
- **The harness, not the model, is the security boundary.** A black-hat can field
  the same frontier models; HALO wins on harness discipline.
