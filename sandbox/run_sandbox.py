...
def build_cmd(script_path, phase, target=None, nonce=None):
    ...
    `nonce` (the per-run verification token) is injected as HALO_NONCE ONLY on the
    live attack phase, so the PoC can echo it back for the output-verification gate
    (breach_confirmed). The isolated test phase never receives it: there is nothing
    to verify against there, and it must not leak into a self-check.
    """
    ...
    if nonce and phase == "attack":
        cmd += ["-e", f"HALO_NONCE={nonce}"]
...
def run(script_path, phase, target=None, timeout=None, nonce=None, _runner=subprocess.run):
    ...
    ALWAYS gets a parseable contract, never a crash with no marker. `nonce` is the
    per-run verification token, forwarded to build_cmd (injected only on the attack
    phase). `_runner` is injectable so this is testable without podman."""
    timeout = DEFAULT_TIMEOUT if timeout is None else timeout
    cmd = build_cmd(script_path, phase, target, nonce=nonce)
    try:
...
def main():
    """CLI entrypoint. halo_tools invokes this file as a subprocess, so the
    verification token only reaches the container when --nonce is parsed here and
    forwarded into run() (which passes it to build_cmd on the attack phase)."""
    ...
    a = p.parse_args()
    out, err, rc = run(a.script, a.phase, a.target, nonce=a.nonce)
