...
class ToolExecutor:
    ...
    def _run_exploit(self, p):
        ...
        # Defense-in-depth: the model is untrusted, so shell-quote every value spliced
        # into the command string. A no-op for normal inputs (plain IPs, hex nonces,
        # /tmp/xxx.py paths) but denies any breakout even if the scope gate is bypassed.
        command = f"python3 {runner} {shlex.quote(script_path)} --phase {phase}"
        if target:
            command += f" --target {shlex.quote(target)}"
        # Per-run verification token: forward it to the sandbox CLI only on the live
        # attack phase, so run_sandbox injects HALO_NONCE and the PoC can echo it for
        # the output-verification gate. Hex from make_nonce(); spliced the same
        # controlled way as --target.
        if phase == "attack" and nonce:
            command += f" --nonce {shlex.quote(nonce)}"
        result = self._execute_command(command)
    ...
    def _run_metasploit(self, p):
        ...
        PRIVATE by construction, per the host isolation constraint: `msfconsole
...
TOOLS = [
    ...
    {"name": "run_exploit", "description": "LAST RESORT: run a custom Python PoC in the isolated sandbox runner. Requires operator approval upstream.",
     "inputSchema": _s("", ["code"], code=_str("Full Python script source."), target=_str("Target ip or ip:port."),
                       ...
                       nonce=_str("Per-run verification token; forwarded to the sandbox on the attack phase."))},
